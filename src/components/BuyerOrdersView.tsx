...
export const BuyerOrdersView: React.FC = () => {
  const { buyerOrders, currentUser, setActiveTab, showToast, language } = useApp();
  ...
  const handleCopy = (mails: string[]) => {
    ...
    setCopied(true);
    showToast(language === 'bn' ? 'মেইল ক্রেডেনশিয়াল কপি করা হয়েছে!' : 'Mail credentials copied!', 'success');
  ...
  const handleDownload = (order: BuyerOrder) => {
    ...
    showToast(language === 'bn' ? 'ফাইল ডাউনলোড হয়েছে' : 'File downloaded', 'success');
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white">
            {language === 'bn' ? 'আমার ক্রয়কৃত অর্ডারসমূহ (My Orders)' : 'My Purchased Orders'}
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            {language === 'bn'
              ? 'মার্কেটপ্লেস থেকে আপনার কেনা সমস্ত মেইল ও ক্রেডেনশিয়াল'
              : 'All Gmail accounts and credentials purchased from marketplace'}
          </p>
        </div>
        <button
        ...
        >
          ...
          <span>{language === 'bn' ? 'নতুন অর্ডার করুন' : 'Order New'}</span>
        </button>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl">
        {myOrders.length === 0 ? (
          <div className="py-16 text-center text-slate-500 space-y-3">
            ...
            <p className="text-sm">
              {language === 'bn' ? 'আপনি এখনো কোনো মেইল অর্ডার করেননি।' : 'You have not ordered any Gmail accounts yet.'}
            </p>
            <button
              onClick={() => setActiveTab('buy')}
              className="px-5 py-2.5 bg-amber-500 text-slate-950 font-bold text-xs rounded-xl inline-flex items-center gap-2"
            >
              <span>{language === 'bn' ? 'মার্কেটপ্লেস ব্রাউজ করুন' : 'Browse Marketplace'}</span>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-800/60 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-700">
                <tr>
                  <th className="py-3 px-4 font-semibold">{language === 'bn' ? 'অর্ডার আইডি' : 'Order ID'}</th>
                  <th className="py-3 px-4 font-semibold">{language === 'bn' ? 'প্যাকেজ নাম' : 'Package Name'}</th>
                  <th className="py-3 px-4 font-semibold">{language === 'bn' ? 'পরিমাণ' : 'Qty'}</th>
                  <th className="py-3 px-4 font-semibold">{language === 'bn' ? 'মূল্য' : 'Price'}</th>
                  <th className="py-3 px-4 font-semibold">{language === 'bn' ? 'ডেলিভারি স্ট্যাটাস' : 'Delivery Status'}</th>
                  <th className="py-3 px-4 font-semibold">{language === 'bn' ? 'তারিখ' : 'Date'}</th>
                  <th className="py-3 px-4 font-semibold text-right">{language === 'bn' ? 'অ্যাকশন' : 'Action'}</th>
                </tr>
              </thead>
                ...
                {myOrders.map(order => (
                  <tr key={order.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-semibold text-white">
                      {order.quantity} {language === 'bn' ? 'টি' : 'pcs'}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right space-x-2">
                      ...
                      >
                        ...
                        <span>{language === 'bn' ? 'মেইল দেখুন' : 'View Mails'}</span>
                      </button>
                      <button
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-xl w-full p-6 shadow-2xl space-y-4">
              ...
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-slate-300">
                  {language === 'bn'
                    ...
                    : `Delivered Mails List (${activeOrderDetails.deliveredMails?.length || 0}):`}
                </span>
                <button
                ...
                >
                  ...
                  <span>{copied ? (language === 'bn' ? 'কপি হয়েছে' : 'Copied') : (language === 'bn' ? 'কপি করুন' : 'Copy All')}</span>
                </button>
              </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              >
                ...
                <span>{language === 'bn' ? 'ফাইল ডাউনলোড করুন (.txt)' : 'Download File (.txt)'}</span>
              </button>
