  ...
  if (isInstalled) {
    return (
      <div
        ...
        title={language === 'bn' ? 'অ্যাপ্লিকেশনটি ইতোমধ্যে ইনস্টল্ড অবস্থায় চলছে' : 'Application is already installed'}
      >
        <Check className="w-3.5 h-3.5" />
        <span>{language === 'bn' ? 'অ্যাপ ইনস্টল্ড (Installed)' : 'App Installed'}</span>
      </div>
    );
  }
  ...
  return (
    <>
      <button
        ...
        title={language === 'bn' ? 'MailFactory মোবাইল বা ডেক্সটপ অ্যাপ হিসেবে ইনস্টল করুন' : 'Install MailFactory as mobile or desktop app'}
      >
        <Smartphone className="w-4 h-4 transition-transform group-hover:scale-110" />
        <span>{language === 'bn' ? 'অ্যাপ ইনস্টল করুন (PWA)' : 'Install App (PWA)'}</span>
        ...
        >
          ...
          >
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-black text-white">
                    {language === 'bn' ? 'অ্যাপ ইনস্টলেশন গাইড' : 'App Installation Guide'}
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    {language === 'bn' ? 'MailFactory PWA ইন্সটল করুন' : 'Install MailFactory PWA'}
                  </p>
                </div>
              </div>
              <button
            ...
            {guidePlatform === 'ios' && (
              <div className="space-y-3 text-xs text-slate-300">
                <p className="text-amber-400 font-semibold text-[11px]">
                  {language === 'bn' ? 'অ্যাপল ডিভাইসে Safari ব্রাউজার ব্যবহার করুন:' : 'On Apple iOS, use Safari browser:'}
                </p>
                <ol className="space-y-2 list-decimal list-inside pl-1 text-[11px] leading-relaxed">
                  ...
                  <li>
                    {language === 'bn' ? (
                      ...
                      <>Tap <strong className="text-emerald-400 font-bold">"Add"</strong> in the top right to install to your home screen!</>
                    )}
                  </li>
                </ol>
              </div>
            ...
            {guidePlatform === 'android' && (
              <div className="space-y-3 text-xs text-slate-300">
                <p className="text-amber-400 font-semibold text-[11px]">
                  {language === 'bn' ? 'Android ডিভাইসে Google Chrome ব্যবহার করুন:' : 'On Android devices, use Google Chrome:'}
                </p>
                <ol className="space-y-2 list-decimal list-inside pl-1 text-[11px] leading-relaxed">
                  <li>
                    {language === 'bn' ? (
                      ...
                      <>Tap the <strong className="text-white">three dots menu (⋮)</strong> in Chrome.</>
                    )}
                  </li>
                  <li>
                    {language === 'bn' ? (
                  ...
                  <li>
                    {language === 'bn' ? (
                      ...
                      <>Confirm <strong className="text-emerald-400 font-bold">Install</strong> to add the icon directly to your home screen.</>
                    )}
                  </li>
                </ol>
              </div>
            ...
            {guidePlatform === 'desktop' && (
              <div className="space-y-3 text-xs text-slate-300">
                <p className="text-amber-400 font-semibold text-[11px]">
                  {language === 'bn' ? 'ডেক্সটপ / ল্যাপটপ ব্রাউজারে:' : 'On Desktop / Laptop browser:'}
                </p>
                <ol className="space-y-2 list-decimal list-inside pl-1 text-[11px] leading-relaxed">
                  <li>
                    {language === 'bn' ? (
                      ...
                      <>Click the <strong className="text-white">Install (⊕)</strong> icon on the right side of the address bar.</>
                    )}
                  </li>
                  <li>
                    {language === 'bn' ? (
                      ...
                      <>Or open browser menu <strong className="text-white">(⋮) &gt; Install Mail Factory</strong>.</>
                    )}
                  </li>
                  <li>
                    {language === 'bn' ? (
                      ...
                      <>Enjoy a standalone, fullscreen high-performance app window.</>
                    )}
                  </li>
                </ol>
              </div>
